  ...
  const handleCurrentLocation = () => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition((position) => {
        alert(`Searching for ${domainName} companies near: ${position.coords.latitude}, ${position.coords.longitude}`);
        setIsLocating(false);
        onClose();
      }, (error) => {
...
const CompanyCard = ({ company, theme }) => (
  ...
  >
    ...
    <div className="flex items-center justify-between pt-6 border-t border-black/5 dark:border-white/5">
      ...
      <a href={`#`} className="text-indigo-500 hover:text-indigo-400 font-bold text-sm flex items-center gap-1">
  ...
  <div className="pt-40 pb-20 px-6 flex justify-center">
    ...
    >
      <div className="text-center mb-10">
        <h2 className="text-4xl font-black tracking-tight mb-2">{mode === 'signin' ? 'Welcome Back' : 'Create Account'}</h2>
      ...
      <form className="space-y-4">
        {mode === 'signup' && (
          <div className="relative">
            ...
            <input type="text" placeholder="Full Name" className="w-full bg-black/10 dark:bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 py-5 focus:outline-none focus:border-indigo-500" />
          </div>
        )}
        <div className="relative">
          <Mail className="absolute left-5 top-1/2 -translate-y-1/2 opacity-30" size={20} />
          <input type="email" placeholder="Email" className="w-full bg-black/10 dark:bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 py-5 focus:outline-none focus:border-indigo-500" />
        </div>
        <div className="relative">
          <Lock className="absolute left-5 top-1/2 -translate-y-1/2 opacity-30" size={20} />
          <input type="password" placeholder="Password" className="w-full bg-black/10 dark:bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 py-5 focus:outline-none focus:border-indigo-500" />
        </div>
        <button className="w-full bg-indigo-600 py-5 rounded-2xl font-black text-white hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20 mt-6 active:scale-95">
          {mode === 'signin' ? 'Sign In' : 'Sign Up'}
        </button>
      </form>
      <p className="text-center mt-8 text-sm font-medium opacity-60">
        {mode === 'signin' ? "Don't have an account?" : "Already a member?"}
        <Link to={mode === 'signin' ? '/signup' : '/signin'} className="text-indigo-500 ml-2 font-bold hover:underline">
